...
struct PeriodicData{
	...
	std::vector<int> assemble(GridData& g) const;
...
void GridMSH(const GridData& g, std::string fn);

void GridMSH(const GridData& g, std::string fn, BNamesFun bnames);

void GridMSH(const GridData& g, std::string fn, PeriodicData pd);

void GridMSH(const GridData& g, std::string fn, BNamesFun bnames, PeriodicData pd);
