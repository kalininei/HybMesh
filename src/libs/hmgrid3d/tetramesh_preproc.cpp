...
SurfacePreprocess::SurfacePreprocess(const Surface::Tree& tree, double split_angle){
	assert(
...
void SurfacePreprocess::assemble_bnd_grid(){
	...
	for (auto& s: ds){
		assert(s.size() > 0);
		Face* f;
		if (s[0]->has_right_cell()) f = s[0]->right.lock()->faces[0].get();
		else f = s[0].get();
...
void SurfacePreprocess::supplement_from_decomposed_surfs(){
	...
	//decomposed_edges
	for (auto& ds: decomposed_surfs){
		vector<vector<HM3D::EdgeData>> de;
		for (auto& s: ds) if (s.size() > 0){
			vector<EdgeData> vds;
...
void SurfacePreprocess::Restore(GridData& g, const VertexData& gvert, const VertexData& svert){
	aa::enumerate_ids_pvec(g.vvert);
	aa::enumerate_ids_pvec(bnd_grid.vvert);
