...
struct vtkcell_expression{
	...
	std::pair<int, int> get_opposite(std::vector<std::vector<int>>& data, int ic){
		...
		int p_oppose = -1;
		for (int i=0; i<data.size(); ++i) if (i!=ic){
			auto fnd = std::find(data[i].begin(), data[i].end(), data[ic][0]);
			if (fnd == data[i].end()) continue;
			auto fndnext = std::next(fnd);
			if (fndnext == data[i].end()) fndnext = data[i].begin();
			if (std::find(data[ic].begin(), data[ic].end(), *fndnext) ==
					data[ic].end()){
				p_oppose = *fndnext;
				break;
			}
			auto fndprev = (fnd == data[i].begin()) ? --data[i].end() : std::prev(fnd);
			if (std::find(data[ic].begin(), data[ic].end(), *fndprev) ==
					data[ic].end()){
				p_oppose = *fndprev;
				break;
			}
