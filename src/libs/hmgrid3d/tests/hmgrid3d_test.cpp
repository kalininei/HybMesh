...
void test10(){
	...
	GridGeom g2 = GGeom::Constructor::ExtractCells(g1, {0});
	{
		...
		add_check(ISEQ(GGeom::Info::Area(g2)*hz, HM3D::SumVolumes(res3.vcells)),
	...
	}
	{
		...
		auto res3 = HM3D::Mesher::UnstructuredTetrahedral(srf3);
		add_check(ISEQ(GGeom::Info::Area(g2)*2*hz, HM3D::SumVolumes(res3.vcells)),
				"2 layers");
	}
	{
		...
		add_check(ISEQ(GGeom::Info::Area(g2)*3*hz, HM3D::Cell::SumVolumes(res3.vcells)),
				"3 layers");
		*/
	}
}
