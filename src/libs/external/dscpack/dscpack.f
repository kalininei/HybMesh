...
C compile with -cpp to enable preprocessor
#define DSCP_MAXN 200
#define DSCP_MAXQ 10000
...
C     ..
   ...
   20 CONTINUE
      ...
      END
C    ----------------------------------------------
      SUBROUTINE DSC_QINIT(M,N,ALFA0,ALFA1,NPTQ,QWORK)
C    ----------------------------------------------
...
C   OUTPUT,AND THE LAST ONE IS A SCRATCH VECTOR USED BY DSC_GAUSSJ.
...
C     ..
      ...
      DO 30 K = 1,M + N
          ...
          IF (K.LE.M) THEN
              ...
              IF (ALFA0(K).GT.0.D0) THEN
                  CALL DSC_GAUSSJ(NPTQ,0.D0,ALPHA,QWORK(ISCR),
     +                            QWORK(INODES), QWORK(IWTS))
   ...
   10             CONTINUE
          ...
          ELSE
              ...
              CALL DSC_GAUSSJ(NPTQ,0.D0,ALPHA,QWORK(ISCR),QWORK(INODES),
     +                    QWORK(IWTS))
          END IF
C
...
C   COMPUTE NODES & WEIGHTS FOR PURE GAUSSIAN QUADRATURE:
      INODES = NPTQ* (M+N) + 1
      IWTS = NPTQ* (2* (M+N)+1) + 1
      CALL DSC_GAUSSJ(NPTQ,0.D0,0.D0,QWORK(ISCR),QWORK(INODES),
     +                QWORK(IWTS))
      RETURN
...
C   OF WPROD AND DSC_QINIT FOR THE REST OF CALLING SEQUENCE.
C
C
C     .. Scalar Arguments ..
...
C
C
C   DSC_CHECK FOR ZERO-LENGTH INTEGRAL:
C     .. Scalar Arguments ..
      DOUBLE COMPLEX WA,WB
      DOUBLE PRECISION PHIA,PHIB,RADIUS,U
      INTEGER IC,KWA,LINEARC,M,N,NPTQ
...
C     .. External Functions ..
      ...
      DOUBLE PRECISION DSC_DIST
      EXTERNAL WQSUM,DSC_DIST
C     ..
C     .. Intrinsic Functions ..
...
C   STEP1:ONE-SIDED G-J QUADRATURE FOR LEFT ENDPT WA:
      R = MIN(1.D0,DSC_DIST(M,N,W0,W1,WA,KWA,IC)/ABS(WB-WA))
      WAA = WA + R* (WB-WA)
...
C   STEP2:ADJOIN INTERVALS OF PURE GAUSS QUADRATURE IF NECESSARY:
   20 IF (R.EQ.1.D0) RETURN
      R = MIN(1.D0,DSC_DIST(M,N,W0,W1,WAA,0,IC)/ABS(WAA-WB))
      WBB = WAA + R* (WB-WAA)
...
*   STEP1:ONE-SIDED G-J QUADRATURE FOR LEFT ENDPT WA:
   30 R = MIN(1.D0,DSC_DIST(M,N,W0,W1,WA,KWA,IC)/ABS(WB-WA))
...
C     ..
C     .. Arrays in Common ..
      DOUBLE COMPLEX W0(DSCP_MAXN),W1(DSCP_MAXN),
     +               Z0(DSCP_MAXN),Z1(DSCP_MAXN)
      DOUBLE PRECISION ALFA0(DSCP_MAXN),ALFA1(DSCP_MAXN),
     +                 PHI0(DSCP_MAXN),PHI1(DSCP_MAXN),
     +                 QWORK(DSCP_MAXQ),UARY(8),VARY(3)
      INTEGER IND(20)
C     ..
C     .. Local Scalars ..
...
C  QWORK     SEE CALLING SEQUENCE DOCUMENTATION IN DSC_QINIT.THE ARRAY
C            MUST HAVE BEEN FILLED BY DSC_QINIT BEFORE CALLING DSCSOLV.
...
C     ..
C     .. Arrays in Common ..
      DOUBLE COMPLEX W02(DSCP_MAXN),W12(DSCP_MAXN),
     +               Z02(DSCP_MAXN),Z12(DSCP_MAXN)
      DOUBLE PRECISION ALFA02(DSCP_MAXN),ALFA12(DSCP_MAXN),
     +                 PHI02(DSCP_MAXN),PHI12(DSCP_MAXN),
     +                 QWORK2(DSCP_MAXQ),UARY(8),VARY(3)
      INTEGER IND(20)
C     ..
C     .. Local Scalars ..
...
C     ..
C     .. Local Arrays ..
      DOUBLE PRECISION DIAG(M+N+2), FJAC(M+N+2,M+N+2),
     +                 FVAL(M+N+2), X(M+N+2),
     +                 QW((N+M+2)*(N+M+2+11)/2)
C     ..
C     .. External Functions ..
...
C      PRINT '(/)'
C      WRITE (6,FMT=*)
C     +  'WANT SCREEN-DISPLAY OF THE RESIDUAL OF THE SYSTEM'
C      WRITE (6,FMT=*)
C     +  '           AS THE ITERATION GOES ON ?            '
C      WRITE (6,FMT=*) '(1 FOR "YES", 2 FOR "NO")'
C      PRINT '(/)'
C      READ (5,FMT=*) ISPRT
C      IF (ISPRT.EQ.1) THEN
C          WRITE (6,FMT=*)
C     +      '# OF ITERATIONS> <MAXIMUM NORM OF THE RESIDUALS'
C
C      ELSE
C          WRITE (6,FMT=*)
C     +      '***TIME FOR OBTAINING A CONVERGED RESULT MAY RANGE'
C          WRITE (6,FMT=*)
C     +      ' FROM SEVERAL SECONDS TO SEVERAL MINUTES OR LONGER'
C          WRITE (6,FMT=*)
C     +      ' DEPENDING ON THE COMPLEXITY OF THE GEOMETRY OF THE'
C          WRITE (6,FMT=*) '    REGION AND THE LOCAL COMPUTING SYSTEM.'
C          WRITE (6,FMT=*) '               SO, BE PATIENT !'
C      END IF
      ISPRT = 0
...
C  SOLVE NONLINEAR SYSTEM WITH HYBRD:
     ...
     +)
C
C  DSC_CHECK ERROR INFORMATION. THE DESCRIPTION OF INFO CAN BE
C  FOUND IN THE DOCUMENTATION OF HYBRD.(INFO=1: SECCESSFUL EXIT)
      IF (INFO /= 1) WRITE (6,FMT=*) 'ERROR IN DSCPACK::HYBRD ',INFO
C
...
C   FIND THE NEAREST PREVERTEX TO THE PT WW0:
   10 CALL DSC_NEARW(M,N,W0,W1,ALFA0,WW0,KNEAR,INEAR)
      IF (INEAR.EQ.0) THEN
          ZA = Z0(KNEAR)
          WA = W0(KNEAR)
...
C  2.GENERATE THE INITIAL VALUE FOR SOLVING ODE:
   30 CALL DSC_NEARZ(M,N,ZS0,ZS1,ALFA0,ZZ,KNZ,INZ)
...
C  5.VERIFICATION:
 ...
 9000 FORMAT (/,'***THE INVERSE EVALUATION FAILED',/,3X,'AT POINT Z=(',
     ...
     +       'SOME ESSENTIAL DIFFICULTIES.')
      END
C    --------------------------------------
      SUBROUTINE DSC_ANGLES(MN,Z01,ALFA01,I01)
C    --------------------------------------
C   COMPUTES THE INTERIOR DSC_ANGLES OF A DOUBLY
...
C   ALFA01: ARRAY CONTAINING INTERIOR TURNING DSC_ANGLES ON RETURN
C
C
C     .. Scalar Arguments ..
      INTEGER I01,MN
...
C     ..
      ...
      END
C   --------------------------------------------------
      SUBROUTINE DSC_NEARW(M,N,W0,W1,ALFA0,W,KNEAR,INEAR)
C   --------------------------------------------------
...
C     ..
      ...
      DO 10 I = 1,M
          D = ABS(W-W0(I))
          IF (D.LE.1.D-11 .OR. D.GE.DSC_DIST .OR.
   ...
   10 CONTINUE
      ...
      DO 20 I = 1,N
          D = ABS(W-W1(I))
          IF (D.LE.1.D-6 .OR. D.GE.DSC_DIST) GO TO 20
          DSC_DIST = D
          KNEAR = I
          INEAR = 1
   20 CONTINUE
      ...
      END
C   --------------------------------------------------
      SUBROUTINE DSC_NEARZ(M,N,Z0,Z1,ALFA0,Z,KNZ,INZ)
C   --------------------------------------------------
...
C     ..
   ...
   10 CONTINUE
      DO 20 I = 1,N
          D = ABS(Z-Z1(I))
          IF (D.LE.1.D-11 .OR. D.GE.DSC_DIST) GO TO 20
          DSC_DIST = D
          KNZ = I
          INZ = 1
   20 CONTINUE
      ...
      DOUBLE PRECISION
C   -----------------------------------
     +  FUNCTION DSC_DIST(M,N,W0,W1,W,KWA,IC)
C   -----------------------------------
C    DETERMINES THE DSC_DISTANCE FROM W TO THE NEAREST SINGULARITY
...
C     ..
   ...
   20 CONTINUE
      ...
      END
C  ---------------------------------------------
      SUBROUTINE DSC_CHECK(ALFA0,ALFA1,M,N,ISHAPE)
C  ---------------------------------------------
C   DSC_CHECKS IF THE INPUT DATA AND PARAMETERS ARE CORRECT.
C   NOTE1:    ANGLE-DSC_CHECKING MAKES SENSE ONLY IF DSC_ANGLES
C             ARE USER-PROVIDED.
C   NOTE2:    USERS ARE RESPONSIBLE FOR DSC_CHECKING THE FOLLOWING:
...
C     ..
   ...
   30 IF (ABS(SUM- (M-2)).LT.EPS) GO TO 40
      WRITE (6,FMT=*)
     +  '***WARNING: SOME DSC_ANGLES FOR OUTER POLYGON ARE WRONG'
      STOP
   ...
   50 CONTINUE
      ...
      WRITE (6,FMT=*)
     +  '***WARNING: SOME DSC_ANGLES FOR INNER POLYGON ARE WRONG'
      STOP
  ...
  110 WRITE (6,FMT=*)
     +  '*** INPUTS ARE DSC_CHECKED WITH NO ERROR BEING FOUND ***'
      RETURN
...
C     .. Local Scalars ..
      ...
      DOUBLE PRECISION D,D1,DSC_DIST,ERRMAX,ERRMIN
...
C
      END
C
      SUBROUTINE DSC_GAUSSJ(N,ALPHA,BETA,B,T,W)
C
...
C
C     SUBROUTINES REQUIRED: DSC_CLASS, DSC_IMTQL2
C
C     REFERENCE:
C
...
C     ..
C     .. External Subroutines ..
      EXTERNAL DSC_CLASS,DSC_IMTQL2
C     ..
      CALL DSC_CLASS(N,ALPHA,BETA,B,T,MUZERO)
      W(1) = 1.0D0
      DO 10 I = 2,N
          W(I) = 0.0D0
   10 CONTINUE
      CALL DSC_IMTQL2(N,T,B,W,IERR)
      DO 20 I = 1,N
          W(I) = MUZERO*W(I)*W(I)
   20 CONTINUE
      RETURN
      ...
      END
      SUBROUTINE DSC_CLASS(N,ALPHA,BETA,B,A,MUZERO)
C
...
C              J J            J   J-1       J-1 J-2
C
C        FOR THE VARIOUS DSC_CLASSICAL (NORMALIZED) ORTHOGONAL POLYNOMIALS,
...
C
      AB = ALPHA + BETA
      ABI = 2.0D0 + AB
      MUZERO = 2.0D0** (AB+1.0D0)*DSC_DGAMMA(ALPHA+1.0D0)*
     +         DSC_DGAMMA(BETA+1.0D0)/DSC_DGAMMA(ABI)
      A(1) = (BETA-ALPHA)/ABI
   ...
   10 CONTINUE
      ...
      END
      SUBROUTINE DSC_IMTQL2(N,D,E,Z,IERR)
C
C     THIS IS A MODIFIED VERSION OF THE EISPACK ROUTINE DSC_IMTQL2.
...
C                EIGENVALUE AFTER 30 ITERATIONS ::::::::::
  ...
  110 RETURN
      ...
      END
      DOUBLE PRECISION FUNCTION DSC_DGAMMA(X)
*
