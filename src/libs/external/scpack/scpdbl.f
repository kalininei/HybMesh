...
c*******************************************************************
c
      complex*16 function wsc(zz,kzz,z0,w0,k0,n,c,z,betam,nptsq,qwork)
c
...
c
c using atan2 instead of log because the latter fails
c at ztmp = (1.0, 0.0). Old code is
c        if (k.eq.ks) ztmp = ztmp / abs(ztmp)
c    1   zsum = zsum + betam(k)*log(ztmp)
      zsum = (0.d0,0.d0)
      do 1 k = 1,n
        ztmp = (1.d0,0.d0) - zz/z(k)
         if (k.eq.ks) then
           zsum = zsum + betam(k)*(0.d0, 1.d0)*
     &            atan2(imag(ztmp), real(ztmp))
         else
           zsum = zsum + betam(k)*log(ztmp)
         end if
    1    continue
