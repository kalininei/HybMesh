...
class CoordinateMap2D{
	...
	//1 - refference x coordinate (lies in geps neighbourhood of real x)
...
public:
	...
	struct Itr_{
		...
		static Itr_ y_search(TPar* p, TL0 it, double y){
			Itr_ ret;
			ret.parent=p;
			ret.level0 = it;
			for (auto i=it->second.begin(); i!=it->second.end(); ++i){
				if (fabs(i->realy - y)<geps){
					ret.level1 = i;
					return ret;
				}
			}
		...
		static Itr_ find_(TPar* p, double x, double y){
			auto f1 = p->sdata.upper_bound(x-geps);
			if (f1 == p->sdata.end()) return end_(p);
			if (fabs(x-f1->first)<geps){
				Itr_ ret = y_search(p, f1, y);
				if (ret != end_(p)) return ret;
				++f1;
				if (fabs(x-f1->first)<geps) return y_search(p, f1, y);
			};
			return end_(p);
		}
	};
	...
	bool add(double x, double y, D val=0){
		auto f1 = sdata.upper_bound(x-geps);
		if (f1 == sdata.end() || fabs(x-f1->first)>=geps){
			auto er = sdata.emplace(x, vector<XData>());
			er.first->second.push_back(XData{x, y, val});
			return true;
		} else {
			if (Iterator::y_search(this, f1, y) != end()) return false;
			auto f2 = f1; ++f2;
			if (fabs(x-f2->first)<geps && Iterator::y_search(this, f2, y)!=end()) return false;
