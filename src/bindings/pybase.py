...
class Hybmesh(object):
    ...
    def __init__(self):
        ...
        self._worker.connection = self._worker.require_connection(
                self.hybmesh_exec_path)
        self._worker.cport = ct.cdll.LoadLibrary(os.path.abspath(
                self.hybmesh_lib_path))
    ...
    class Contour2D(object):
        def __init__(self, sid, worker):
            super(Hybmesh.Contour2D, self).__init__()
            self.sid = sid
            self._worker = worker
    ...
    class Grid2D(object):
        def __init__(self, sid, worker):
            super(Hybmesh.Grid2D, self).__init__()
            self.sid = sid
            self._worker = worker
    ...
    class Surface3D(object):
        def __init__(self, sid, worker):
            super(Hybmesh.Surface3D, self).__init__()
            self.sid = sid
            self._worker = worker
    ...
    class Grid3D(object):
        def __init__(self, sid, worker):
            super(Hybmesh.Grid3D, self).__init__()
            self.sid = sid
            self._worker = worker
